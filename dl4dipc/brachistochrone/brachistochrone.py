...
class MLP(nn.Module):
    ...
    def value_and_grad(self, x):
        ...
        return (
            y,
            torch.autograd.grad(
                y, x, grad_outputs=torch.ones(x.shape[0]), create_graph=True
            )[0],
        )
...
if __name__ == "__main__":
    ...
    para = np.append(
        solver(v0, g, y1), v0 ** 2 / (2 * g)
    )  # exact solution as a reference
    ...
    for epoch in range(100):
        ...
        optimizer.step()
        print(
            "step = %d" % epoch, "time = %.5f" % loss.item(), " (exact = %.5f)" % tbest
        )
        plot(model, para)
