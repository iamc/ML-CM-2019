...
class RBM:
    ...
    def __init__(
        ...
        # pairwise weights:
        default = tf.random_normal(
            shape=(self.num_visible, self.num_hidden), mean=0, stddev=0.05
        )
    ...
    def all_hidden_states(self):
        if self._all_hidden_states is None:
            assert (
                self.num_hidden <= self.max_feasible_for_log_pf
            ), "cannot generate all hidden states for num_hidden > {}".format(
                self.max_feasible_for_log_pf
            )
            self._all_hidden_states = np.array(
                list(it.product([0, 1], repeat=self.num_hidden)), dtype=np.float32
            )
    ...
    def p_of_h_given(self, v):
        ...
        return tf.nn.sigmoid(
            tf.matmul(v, self.weights) + tf.transpose(self.hidden_bias)
        )
    ...
    def p_of_v_given(self, h):
        # type: (tf.Tensor) -> tf.Tensor
        return tf.nn.sigmoid(
            tf.matmul(h, self.weights, transpose_b=True)
            + tf.transpose(self.visible_bias)
        )
    ...
    def energy(self, hidden_samples, visible_samples):
        # type: (tf.Tensor, tf.Tensor) -> tf.Tensor
        return (
            -tf.matmul(hidden_samples, self.hidden_bias)  # b x m * m x 1
            - tf.matmul(visible_samples, self.visible_bias)  # b x n * n x 1
            - tf.reduce_sum(
                tf.matmul(visible_samples, self.weights) * hidden_samples, 1
            )
        )
    ...
    def neg_log_likelihood_forGrad(self, visible_samples, num_gibbs=2):
        ...
        expectation_from_data = tf.reduce_mean(
            self.energy(hidden_samples, visible_samples)
        )

        model_hidden, model_visible = self.stochastic_maximum_likelihood(num_gibbs)
        expectation_from_model = tf.reduce_mean(
            self.energy(model_hidden, model_visible)
        )
    ...
    def neg_log_likelihood(self, visible_samples, log_Z):
        free_energy = tf.matmul(visible_samples, self.visible_bias) + tf.reduce_sum(
            tf.nn.softplus(
                tf.matmul(visible_samples, self.weights)
    ...
    def exact_log_partition_function(self):
        with tf.name_scope("exact_log_Z"):
            ...
            first_term = tf.matmul(
                self.all_hidden_states, self.hidden_bias, name="first_term"
            )
            with tf.name_scope("second_term"):
                second_term = tf.matmul(
                    self.weights, self.all_hidden_states, transpose_b=True
                )
                second_term = tf.nn.softplus(tf.add(self.visible_bias, second_term))
                second_term = tf.transpose(
                    tf.reduce_sum(second_term, reduction_indices=[0], keep_dims=True)
                )
            exponent = tf.cast(
                first_term + second_term, dtype=tf.float64, name="exponent"
            )
            # exponent_mean = tf.reduce_mean(exponent)
            exponent_mean = tf.reduce_max(exponent)

            return (
                tf.log(tf.reduce_sum(tf.exp(exponent - exponent_mean))) + exponent_mean
            )
