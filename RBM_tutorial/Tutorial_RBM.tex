...
\opt{nosoln}{\title{Tutorial: \\Learning thermodynamic observables with \\restricted Boltzmann machines \vspace*{-6mm}}}
\opt{soln}{\title{Tutorial \textcolor{JungleGreen}{Solutions}: \\Learning thermodynamic observables with \\restricted Boltzmann machines \vspace*{-6mm}}}
...
\texttt{tutorial{\textunderscore}sample{\textunderscore}ising2d.py} %and \texttt{tutorial5{\textunderscore}plot{\textunderscore}results.py}.
...
%%%%%%%%%%%%%% (a) %%%%%%%%%%%%%%
\item Start by examining Figure 1 at the end of this document. How do the thermodynamic observables generated from the RBM depend on the number of hidden units $n = n_h$? How does the accuracy of each observables vary with the distance from the critical temperature $T_\text{c}$? 
...
%%%%%%%%%%%%%% (c) %%%%%%%%%%%%%%
\item Examine the parameters \texttt{learning{\textunderscore}rate{\textunderscore}start}, \texttt{bsize}, \texttt{num{\textunderscore}gibbs} and  \texttt{num{\textunderscore}samples} and 
understand how each are used to train the RBM.
...
Are your discrepancies similar to those found in Figure 1?

\hint{In order to check if your code is working, you can try calculating the observable quantities for the training configurations in the folder \texttt{MC{\textunderscore}results} 
to varify that you get the same results as in Table~\ref{tab}.}

%%%%%%%%%%%%%% (f) %%%%%%%%%%%%%%
\item Repeat parts b), d) and e) for other values of the number of hidden units $n$.
Once again consider how your results compare with Figure 1.

\note{The exact (log) partition function requires a trace over all hidden units, which becomes intractable for some number of hidden units.  Currently \texttt{max{\textunderscore}feasible{\textunderscore}for{\textunderscore}log{\textunderscore}pf} is set to $n_h=24$.  For larger number of hidden units, you will need to disable this calculation (and the plotting) in  \texttt{tutorial{\textunderscore}train{\textunderscore}ising2d.py} .}
...
Once again examine how your results compare with Figure 1.
