...
T = (
    2.269
)  # a temperature for which there are MC configurations stored in Data_ising2d/MC_results
...
learning_rate_start = (
    1e-3
)  # the learning rate will start at this value and decay exponentially
...
def save_parameters(sess, rbm):
    weights, visible_bias, hidden_bias = sess.run(
...
trainFileName = (
    "Data_ising2d/MC_results/ising2d_L" + str(L) + "_T" + str(T) + "_train.txt"
)
xtrain = np.loadtxt(trainFileName)
testFileName = (
    "Data_ising2d/MC_results/ising2d_L" + str(L) + "_T" + str(T) + "_test.txt"
)
...
placeholders.visible_samples = tf.placeholder(
    tf.float32, shape=(None, num_visible), name="v"
)  # placeholder for training data

total_iterations = 0  # starts at zero
ops.global_step = tf.Variable(
    total_iterations, name="global_step_count", trainable=False
)
...
for ii in range(nsteps):
    ...
    if num_steps % iterations_per_epoch == 0:
        ...
        nll_test = sess.run(
            NLL,
            feed_dict={
                placeholders.visible_samples: xtest_randomized,
                placeholders.logZ: lz,
            },
        )
        nll_test_list.append(nll_test)

        print(
            "Epoch = %d, Nsteps = %d, NLL on test data = %.6f"
            % (epochs_done, num_steps, nll_test)
        )
